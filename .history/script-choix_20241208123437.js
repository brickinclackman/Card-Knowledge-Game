...
function findCardByName(name) {
    // Extraire les termes du nom (ex: "Carreaux As")
    const parts = name.split(' ');
    if (parts.length !== 2) return null;

    const suit = parts[1];  // Le deuxième terme, par exemple "Carreaux"
    const value = parts[0];  // Le premier terme, par exemple "As"

    // Chercher la carte dans le tableau de cartes
    return cards.find(card => card.suit === suit && card.value === cardValue(value));
}
