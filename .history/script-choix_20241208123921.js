...
// Fonction pour chercher une carte par son nom, en fonction du "suit" et "value"
function findCard(cardName) {
    // Séparer le nom de la carte en deux parties (suit et value)
    const [suitName, valueName] = cardName.split(' '); // "Carreaux As" -> ["Carreaux", "As"]

    // Chercher dans le tableau des cartes
    for (let card of cardsData.cards) {
        // Comparer la suit et la value
        if (card.suit === suitName && card.value === valueName) {
            return card; // Retourne la carte si trouvée
        }
    }
    ...
    if (questions) {
        ...
        drawCardButton.addEventListener('click', () => {
            if(Array.isArray(cards) && cards.length > 0){
                ...
                const foundCard = findCardByName(cardName);
