...
// Fonction pour rechercher une carte à partir d'un nom comme "Carreaux As"
function findCard(name) {
    // Séparer la chaîne en deux parties : suit et value
    const parts = name.split(" "); // Diviser par un espace
    const suit = parts[0]; // Premier terme (par ex. "Carreaux")
    const value = parts[1]; // Deuxième terme (par ex. "As")

    // Chercher la carte dans le tableau "cards"
    const card = cards.find(card => card.suit === suit && card.value === value);

    if (card) {
        return card; // Si la carte est trouvée, la retourner
    } else {
        return null; // Si aucune carte ne correspond, retourner null
    }
}
    ...
    if (questions) {
        ...
        drawCardButton.addEventListener('click', () => {
            if(Array.isArray(cards) && cards.length > 0){
                ...
                // Nom à rechercher (par exemple, "Carreaux As")
                const cardName = randomSuit +" "+ randomCard;                

                // Appel de la fonction pour rechercher la carte dans le fichier JSON
                const foundCard = findCard(cardName);

                console.log(cardName,foundCard);

                if (foundCard) {
                    console.log("Carte trouvée :", foundCard);
                    // Vous pouvez afficher les données de la carte trouvée
                    // Par exemple, afficher l'image et les détails :
                    document.getElementById('cardContainer').innerHTML = `
                        <img src="${foundCard.image}" alt="${foundCard.name}">
                        <p>${foundCard.name} de ${foundCard.suit} (Valeur: ${foundCard.value})</p>
                    `;
